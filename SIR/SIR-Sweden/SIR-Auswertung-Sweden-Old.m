...
population_sweden  = 10230000; % Statista.com (2018)
N                  = population_sweden;

% Step 1.2: Load Infected Cases

[infected1,infected2] = textread('Sweden-Infected.csv','%s;%f','delimiter',';');
infected2             = infected2(end:-1:1);
dates                 = (1:1:length(infected2))';

% Step 1.3: Load Recovered Cases

[recovered1,recovered2] = textread('Sweden-Recovered.csv','%s;%f','delimiter',';');
recovered2              = recovered2(end:-1:1);

% Step 1.4: Load Dead Cases

[dead1, dead2] = textread('Sweden-Dead.csv','%s;%f','delimiter',';');
dead2          = dead2(end:-1:1);
...
Beta_Mean   = sum(Beta(2:1:end))/(length(Beta)-2);
Beta_Median = median(Beta(2:1:end));
Beta_SD     = ((1/(length(Beta)-2))*sum((Beta(2:1:end)-Beta_Mean)).^2)^0.5;
...
Beta_mod     = Beta(35:1:end);
x_Beta_SSD   = fminsearch (@(x) ( sum( abs( x(1)*exp(-(((dates_mod-x(2))/x(3)).^2)) - Beta_mod ) ) ),[0.02;90.0;20.0]);
...
lt01 = title("Sweden: People vs. Dates", "fontsize", 14);
...
lt02 = title("Sweden: Transfer Rates vs. Dates", "fontsize", 14);
...
lt03 = title("Sweden: Alpha vs. Dates", "fontsize", 14);
...
ylim([0.0 0.06]);
lt04 = title("Sweden: Beta vs. Dates", "fontsize", 14);
...
plot(dates, Beta_Median_Vec, 'color', 'cyan', '-@');
hold on
plot(dates(35:1:end), Beta_SSD(35:1:end), 'color', 'magenta', '-@');
ylim([0.0 0.06]);
lt04 = title("Sweden: Beta vs. Dates", "fontsize", 14);
xt04 = xlabel("t", "fontsize", 14);
yt04 = ylabel("Transfer Rate Beta", "fontsize", 14);
