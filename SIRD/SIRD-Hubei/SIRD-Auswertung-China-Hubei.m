...
Beta_Mean   = sum(Beta(Beta>=0.0))/(length(Beta>=0.0)-2);
Beta_Median = median(Beta(Beta>=0.0));
Beta_SD     = ((1/(length(Beta>=0.0)-2))*sum((Beta(Beta>=0.0)-Beta_Mean)).^2)^0.5;
...
Gamma_mod       = Gamma(Gamma<Gamma_threshold);
Gamma_Mean      = sum(Gamma_mod(2:1:end))/(length(Gamma_mod)-2);
Gamma_Median    = median(Gamma_mod(2:1:end));
Gamma_SD        = ((1/(length(Gamma_mod)-2))*sum((Gamma_mod(2:1:end)-Gamma_Mean)).^2)^0.5;
...
x_Alpha_SSD = fminsearch (@(x) ( sum( abs((x(1)*exp(-x(2)*dates) - Alpha) )) ),[0.5;0.2]);
...
lt01 = title("Hubei (China): People vs. Dates", "fontsize", 14);
...
lt02 = title("Hubei (China): Transfer Rates vs. Dates", "fontsize", 14);
...
hold off
plot(dates, Alpha, 'color', 'red', '-@');
lt03 = title("Hubei (China): Alpha vs. Dates", "fontsize", 14);
xt03 = xlabel("t", "fontsize", 14);
yt03 = ylabel("Transfer Rate Alpha", "fontsize", 14);
hold on
plot(dates, Alpha_SSD, 'color', 'magenta', '-@');
ylim([0.0 0.4]);
...
ylim([0.0 0.4]);
lt04 = title("Hubei (China): Beta vs. Dates", "fontsize", 14);
...
ylim([0.0 0.05]);
lt05 = title("Hubei (China): Gamma vs. Dates", "fontsize", 14);
