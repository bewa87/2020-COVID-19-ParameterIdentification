...
population_italy   = 60480000; % Statista.com (2018)
N                  = population_italy;

% Step 1.2: Load Infected Cases

[infected1,infected2] = textread('Italy-Infected.csv','%s;%f','delimiter',';');
infected2             = infected2(end:-1:1);
dates                 = (1:1:length(infected2))';

% Step 1.3: Load Recovered Cases

[recovered1,recovered2] = textread('Italy-Recovered.csv','%s;%f','delimiter',';');
recovered2              = recovered2(end:-1:1);

% Step 1.4: Load Dead Cases

[dead1, dead2] = textread('Italy-Dead.csv','%s;%f','delimiter',';');
dead2          = dead2(end:-1:1);
...
Beta_Mean   = sum(Beta(Beta>=Beta_Lower_Threshold & Beta<=Beta_Upper_Threshold))/(length(Beta(Beta>=Beta_Lower_Threshold & Beta<=Beta_Upper_Threshold))-1);
Beta_Median = median(Beta(Beta>=Beta_Lower_Threshold & Beta<=Beta_Upper_Threshold));
Beta_SD     = ((1/(length(Beta(Beta>=Beta_Lower_Threshold & Beta<=Beta_Upper_Threshold))-1))*sum((Beta(Beta>=Beta_Lower_Threshold & Beta<=Beta_Upper_Threshold)-Beta_Mean)).^2)^0.5;
...
dates_mod   = dates(30:1:end);
Alpha_mod   = Alpha(30:1:end);
...
lt01 = title("Italy: People vs. Dates", "fontsize", 14);
...
lt02 = title("Italy: Transfer Rates vs. Dates", "fontsize", 14);
...
lt03 = title("Italy: Alpha vs. Dates", "fontsize", 14);
xt03 = xlabel("t", "fontsize", 14);
yt03 = ylabel("Transfer Rate Alpha", "fontsize", 14);
hold on
plot(dates(30:1:end), Alpha_SSD(30:1:end), 'color', 'magenta', '-@');
ylim([0.0 0.8]);
...
ylim([0.0 0.4]);
lt04 = title("Italy: Beta vs. Dates", "fontsize", 14);
